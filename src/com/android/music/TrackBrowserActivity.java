...
public class TrackBrowserActivity extends ListActivity implements View.OnCreateContextMenuListener, MusicUtils.Defs,
    ...
    public void onCreate(Bundle icicle) {
        ...
        mCursorCols = new String[] { BaseColumns._ID, MediaColumns.TITLE, MediaColumns.DATA, AudioColumns.ALBUM,
                AudioColumns.ARTIST, AudioColumns.ARTIST_ID, AudioColumns.DURATION };
        mPlaylistMemberCols = new String[] { MediaStore.Audio.Playlists.Members._ID, MediaColumns.TITLE,
                MediaColumns.DATA, AudioColumns.ALBUM, AudioColumns.ARTIST, AudioColumns.ARTIST_ID,
                AudioColumns.DURATION, MediaStore.Audio.Playlists.Members.PLAY_ORDER,
                MediaStore.Audio.Playlists.Members.AUDIO_ID, AudioColumns.IS_MUSIC };
    ...
    public void init(Cursor newCursor, boolean isLimited) {
        ...
        if ("nowplaying".equals(mPlaylist)) {
            ...
            if (key != null) {
                int keyidx = mTrackCursor.getColumnIndexOrThrow(AudioColumns.ARTIST_ID);
    ...
    private void setTitle() {
        ...
        if (mAlbumId != null) {
            ...
            if (numresults > 0) {
                mTrackCursor.moveToFirst();
                int idx = mTrackCursor.getColumnIndexOrThrow(AudioColumns.ALBUM);
                ...
                String where = AudioColumns.ALBUM_ID + "='" + mAlbumId + "' AND " + AudioColumns.ARTIST_ID + "="
                        + mTrackCursor.getLong(mTrackCursor.getColumnIndexOrThrow(AudioColumns.ARTIST_ID));
                Cursor cursor = MusicUtils.query(this, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
                        new String[] { AudioColumns.ALBUM }, where, null, null);
                if (cursor != null) {
    ...
    private boolean isMusic(Cursor c) {
        ...
        if (MediaStore.UNKNOWN_STRING.equals(album) && MediaStore.UNKNOWN_STRING.equals(artist) && title != null
                && title.startsWith("recording")) // not music
            return false;
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menuInfoIn) {
        ...
        mCurrentAlbumName = mTrackCursor.getString(mTrackCursor.getColumnIndexOrThrow(AudioColumns.ALBUM));
        mCurrentArtistNameForAlbum = mTrackCursor.getString(mTrackCursor.getColumnIndexOrThrow(AudioColumns.ARTIST));
        mCurrentTrackName = mTrackCursor.getString(mTrackCursor.getColumnIndexOrThrow(MediaColumns.TITLE));
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (mPlaylist != null && event.getMetaState() != 0 && event.getAction() == KeyEvent.ACTION_DOWN) {
    ...
    private void moveItem(boolean up) {
        int curcount = mTrackCursor.getCount();
        int curpos = mTrackList.getSelectedItemPosition();
        if (up && curpos < 1 || !up && curpos >= curcount - 1) return;
    ...
    protected void onListItemClick(ListView l, View v, int position, long id) {
        if (mTrackCursor.getCount() == 0) return;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case SHUFFLE_ALL:
            ...
            cursor = MusicUtils.query(this, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
                    new String[] { BaseColumns._ID }, AudioColumns.IS_MUSIC + "=1", null,
    ...
    private Cursor getTrackCursor(TrackListAdapter.TrackQueryHandler queryhandler, String filter, boolean async) {
        ...
        if (queryhandler == null) throw new IllegalArgumentException();
        ...
        if (mGenre != null) {
            ...
            if (mAlbumId != null) {
                where.append(" AND " + AudioColumns.ALBUM_ID + "=" + mAlbumId);
                mSortOrder = AudioColumns.TRACK + ", " + mSortOrder;
            }
            if (mArtistId != null) {
                where.append(" AND " + AudioColumns.ARTIST_ID + "=" + mArtistId);
            }
    ...
    private class NowPlayingCursor extends AbstractCursor {
        ...
        private void makeNowPlayingCursor() {
            ...
            mCurrentPlaylistCursor = MusicUtils.query(TrackBrowserActivity.this,
                    MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, mCols, where.toString(), null, BaseColumns._ID);
        ...
        public boolean onMove(int oldPosition, int newPosition) {
            ...
            if (mNowPlaying == null || mCursorIdxs == null || newPosition >= mNowPlaying.length) return false;
        ...
        public boolean removeItem(int which) {
            try {
                if (mService.removeTracks(which, which) == 0) return false; // delete
                                                                            // failed
                int i = which;
                mSize--;
                while (i < mSize) {
    ...
    static class TrackListAdapter extends SimpleCursorAdapter implements SectionIndexer {
        ...
        public void bindView(View view, Context context, Cursor cursor) {
            ...
            if (mIsNowPlaying && cursor.getPosition() == id || !mIsNowPlaying && !mDisableNowPlayingIndicator
        ...
        public Object[] getSections() {
            if (mIndexer != null) return mIndexer.getSections();
            else return new String[] { " " };
        }

        @Override
        public int getPositionForSection(int section) {
            if (mIndexer != null) return mIndexer.getPositionForSection(section);
            return 0;
        }
